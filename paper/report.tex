...
\caption{Additional ablations and curves (95\% CI bands from the Monte Carlo seeds). Left: operating-point sweep comparing baseline CSC to an NDWI-removed fallback (EVI/LST-only); horizontal axis limited to 0--5\% FP rate for readability. Middle: sparse Stage~1 cadence (screening every $N$th pass) illustrating the recall trade with under-sampling. Right: sensitivity to effective cloud-mask strictness (cloud-flag rate from 0.05 to 0.75) as a proxy for a more/less conservative onboard mask.}
