...
\caption{Additional ablations and curves (95\% CI bands where applicable). Left: operating-point sweep comparing baseline CSC to an NDWI-removed fallback (EVI/LST-only); horizontal axis limited to 0--5\% FP rate. Middle: sparse Stage~1 cadence; recall often remains at ceiling under the synthetic benchmark while \emph{seasonal-average compute} (right axis) shows the cost of under-sampling Stage~1. Right: $\pm30\%$ CSC weight/saturation grid (horizontal tick labels: $w$=weight scale, $s$=saturation scale); recall stays within 99--100.5\% while \emph{FP rate} (right axis) varies across calibration corners.}
\label{fig:additional-ablations}
\end{figure}

A matched-recall ablation that removes the Bayesian belief memory drives seasonal-average compute from 49.2\% to 76.2\% (a 27 pp increase), while changing false-positive rate only slightly from 1.6\% to 1.7\%.
...
If selected as a finalist, the three actions before the June pitch are: (1)~run the replay on a live MODIS tile for a live demonstration; (2)~obtain a first-pass Loft Orbital hosted-payload cost estimate; (3)~conduct two farmer interviews to validate the $\leq$90-min latency and \$4/ha price assumptions.
