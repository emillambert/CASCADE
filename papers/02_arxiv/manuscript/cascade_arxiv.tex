...
\title{CASCADE: A Software Framework and Synthetic Benchmark for Onboard Anomaly Triage in Earth Observation}
...
\label{eq:csc}
\end{equation}
clipped to $[0,1]$, where $[x]_+=\max(x,0)$. Here $\sigma_E$, $\sigma_L$, and $\sigma_N$ are fixed normalizers (EVI~\cite{huete2002modis}, LST~\cite{wan1996generalized} [K], NDWI~\cite{gao1996ndwi}) that convert raw drops/rises to standardized anomaly magnitudes, and $s_E$, $s_L$, and $s_N$ are saturation constants that set the anomaly level at which each channel alone would reach a clipped value of $1.0$. The default weights are $0.578$, $0.246$, and $0.176$ for EVI, LST, and NDWI respectively.
