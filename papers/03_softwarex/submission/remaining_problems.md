...
This file tracks only unresolved, conditional, or externally blocked items for the CASCADE SoftwareX submission. Confirmed-good and fixed-local items have been removed.
...
- Next action: after review-ready content is pushed to the chosen GitHub repository, create and publish `v1.0.0`, verify the release URL resolves, then recheck C2 and `Current executable software version`.
...
(cd papers/softwarex/manuscript && latexmk -pdf -interaction=nonstopmode -halt-on-error cascade_softwarex.tex)
perl /tmp/cascade-texcount/texcount.pl -inc -sum papers/softwarex/manuscript/cascade_softwarex.tex
pdftotext papers/softwarex/manuscript/cascade_softwarex.pdf - | rg '\b(nite|ight|in uence|xture|xtures|rst|nal|speci c|con rm|veri c|satis es|modi ed)\b'
```
