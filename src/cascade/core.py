...
class CSCDefaults:
    ...
    def weights(self) -> tuple[float, float, float]:
        return (self.evi_weight, self.lst_weight, self.ndwi_weight)

    def saturations(self) -> dict[str, float]:
        return {
            "evi": self.evi_saturation,
            "lst": self.lst_saturation,
            "ndwi": self.ndwi_saturation,
        }

    def as_compute_kwargs(self) -> dict[str, float]:
        return {
...
CSC_DEFAULTS = CSCDefaults(
    evi_weight=0.577777,
    lst_weight=0.246039,
    ndwi_weight=0.176184,
    evi_saturation=7.0,
    lst_saturation=4.956382,
    ndwi_saturation=6.0,
)
CSC_ALERT_THRESHOLD_DEFAULT = 0.615018
CSC_WEIGHTS = CSC_DEFAULTS.weights()
CSC_SIGMAS = {
    "evi": 0.042,
    "lst": 1.20,
    "ndwi": 0.05,
}
CSC_SATURATION_DEFAULTS = CSC_DEFAULTS.saturations()
...
def compute_csc(
    ...
    With NDWI available:
      CSC = w_evi * clamp(evi_drop / (evi_sat*sigma_evi)) +
            w_lst * clamp(lst_rise / (lst_sat*sigma_lst)) +
            w_ndwi * clamp(ndwi_drop / (ndwi_sat*sigma_ndwi))
    ...
    if ndwi_t is None or ndwi_base is None:
        legacy_evi_term = np.clip(
            np.maximum(0.0, (evi_base - evi_t) / sigma_e)
            / max(LEGACY_CSC_SATURATION_DEFAULTS["evi"], 1e-9),
            0,
            1,
        )
        legacy_lst_term = np.clip(
            np.maximum(0.0, (lst_t - lst_base) / sigma_l)
            / max(LEGACY_CSC_SATURATION_DEFAULTS["lst"], 1e-9),
            0,
            1,
        )
        legacy_evi_weight, legacy_lst_weight = _normalized_weights(LEGACY_CSC_WEIGHTS)
        csc = legacy_evi_weight * legacy_evi_term + legacy_lst_weight * legacy_lst_term
