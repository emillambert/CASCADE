...
Synthetic validation:
  ...
  120 passes, 21 benchmark anomaly events, and 9 benign confounders per seed.
"""
...
from cascade.paths import BUILD_BENCHMARK_DIR
...
ADDITIONAL_ABLATION_DEFAULTS = {
    "n_seeds": int(
        _env_any(
            "CASCADE_ADDITIONAL_ABLATION_SEEDS",
            "MASFE_ADDITIONAL_ABLATION_SEEDS",
            default="100",
        )
    ),
    "n_patches": int(
        _env_any(
            "CASCADE_ADDITIONAL_ABLATION_PATCHES",
            "MASFE_ADDITIONAL_ABLATION_PATCHES",
            default="1000",
        )
    ),
    "n_t": 120,
...
def _default_alert_threshold_sweep() -> list[float]:
    return sorted(
        {
            0.40,
            0.45,
            ...
            0.65,
            0.70,
            round(float(CSC_ALERT_THRESHOLD_DEFAULT), 3),
        }
    )


ALERT_THRESH_SWEEP = _default_alert_threshold_sweep()
WEIGHT_SWEEP = [0.70, 1.00, 1.30]
SATURATION_SWEEP = [0.70, 1.00, 1.30]
BENCHMARK_OUTPUT_DIR = BUILD_BENCHMARK_DIR
...
def simulate(
    ...
    *,
    ...
    detection_threshold = (
        getattr(policy, "csc_alert_thr", CSC_ALERT_THRESHOLD_DEFAULT)
        if policy
        else CSC_ALERT_THRESHOLD_DEFAULT
    )
    ...
    for t in range(n_t):
        ...
        if policy_name == "CASCADE_MDP" and action == "FUSE" and policy.should_priority_downlink(state, new_csc):
...
def build_seed_record(seed: int, raw: Dict, fixed: Dict, cascade: Dict, data: Dict) -> dict:
    def pct_save(a, b):
        return (1 - a / b) * 100.0

    fp_rate = cascade["fp"] / max(cascade["tp"] + cascade["fp"], 1) * 100.0
    return {
        "seed": seed,
        "downlink_reduction_vs_raw_pct": pct_save(cascade["data_mb"], raw["data_mb"]),
        "downlink_reduction_vs_fixed_pct": pct_save(cascade["data_mb"], fixed["data_mb"]),
        "energy_saving_vs_raw_pct": pct_save(cascade["energy"], raw["energy"]),
        "energy_saving_vs_fixed_pct": pct_save(cascade["energy"], fixed["energy"]),
        "science_retention_pct": cascade["detected_patches"] / max(len(data["dis_idx"]), 1) * 100.0,
        "alert_precision_pct": 100.0 - fp_rate,
        "false_positive_rate_pct": fp_rate,
        "n_priority_alerts": float(cascade["n_alerts"]),
        "min_battery_soc": float(cascade["min_batt"]),
        "cloud_obscured_passes": float(cascade["cloud_passes"]),
        "seasonal_average_compute_utilisation_pct": float(cascade["seasonal_average_compute_pct"]),
    }
...
def _benchmark_run_log_path() -> Path:
    return BENCHMARK_OUTPUT_DIR / "benchmark_run.log"


def _benchmark_run_log_open() -> None:
    """Truncate and open ``build/benchmark/benchmark_run.log`` for this process."""
    global _BENCHMARK_LOG_FP
    BENCHMARK_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    path = _benchmark_run_log_path()
    _BENCHMARK_LOG_FP = path.open("w", encoding="utf-8")
    _BENCHMARK_LOG_FP.write(f"# CASCADE benchmark run started {datetime.now().isoformat(timespec='seconds')}\n")
...
def _benchmark_log_line(msg: str) -> None:
    """Append one line to the benchmark run log when that file is open."""
    if _BENCHMARK_LOG_FP is not None:
...
def _benchmark_quiet() -> bool:
    ...
    return _env_any(
        "CASCADE_BENCHMARK_QUIET",
        "MASFE_BENCHMARK_QUIET",
        "CASCADE_MONTE_CARLO_QUIET",
...
def run_monte_carlo(
    ...
    if not quiet:
        ...
        if use_tqdm:
            seed_source = _progress_wrap(
                datasets,
                desc="Monte Carlo (RAW+FIXED+CASCADE/seed)",
                unit="seed",
                total=n_total,
                leave=True,
            )
            ...
            print(f"Monte Carlo: starting {n_total} seeds (RAW + FIXED + CASCADE per seed)...", file=sys.stderr, flush=True)
    else:
        seed_source = datasets
        use_tqdm = False
    ...
    for seed, data in enumerate(seed_source):
        ...
        cascade = simulate("CASCADE_MDP", CASCADEPolicy(), data, cfg, outer_seed=seed)

        seed_records.append(build_seed_record(seed, raw, fixed, cascade, data))
        raw_runs.append(raw)
        fixed_runs.append(fixed)
        cascade_runs.append(cascade)
    ...
    metrics = {
        ...
        "seasonal_average_screen_gsd_m": clean_round(
            sum(
                ...
                for action_name, metrics_block in summarize_action_mix([run["action_dist"] for run in cascade_runs], n_t).items()
            ) / max(n_t, 1),
            1,
        ),
        "seasonal_priority_share_pct": clean_round(
            summarize_action_mix([run["action_dist"] for run in cascade_runs], n_t)
            .get("FUSE_PRIORITY", {})
            .get("pct_mean", 0.0),
            1,
        ),
        ...
        "monte_carlo": {
            ...
            "policy_summary": {
                ...
                "CASCADE_MDP": aggregate_policy_results("CASCADE_MDP", cascade_runs, n_t),
            },
        },
    }
    return metrics
...
def run_ablation_analysis(cfg: Config, datasets: list[Dict], baseline_metrics: dict) -> dict:
    ...
    default_eval = evaluate_policy(
        "ABLATE_NO_BELIEF",
        lambda: AblateNoBeliefPolicy(csc_alert_thr=CSC_ALERT_THRESHOLD_DEFAULT),
        cfg,
        datasets,
    )
    default_recall = round(default_eval["stats"]["science_retention_pct"]["mean"], 1)
    selected_threshold = float(CSC_ALERT_THRESHOLD_DEFAULT)
    ...
    if default_recall != target_recall:
        for threshold in ALERT_THRESH_SWEEP:
            ...
            sweep_results[_format_threshold_key(threshold)] = round_stats(eval_result["stats"])
            if round(recall_mean, 1) >= target_recall:
                selected_threshold = threshold
                selected_eval = eval_result
                break
    else:
        sweep_results[_format_threshold_key(CSC_ALERT_THRESHOLD_DEFAULT)] = round_stats(
            default_eval["stats"]
        )
    ...
    return {
        ...
        "baseline_cascade_false_positive_rate_pct": round(float(baseline_fp), 3),
        "selected_alert_threshold": round(selected_threshold, 3),
...
def run_evi_only_baseline(cfg: Config, datasets: list[Dict], baseline_metrics: dict) -> dict:
    ...
    for thr in EVI_ONLY_THRESH_SWEEP:
        ...
        sweep_results[_format_threshold_key(thr)] = round_stats(eval_result["stats"])
        if round(recall_mean, 1) < target_recall:
            continue
...
def write_baselines_comparison_table(
    ...
    cascade = mc["policy_summary"]["CASCADE_MDP"]
    ...
    full_compute = float(cascade["seasonal_average_compute_pct_mean"])
    lines.append(
...
def run_roc_sweep(cfg: Config, datasets: list[Dict]) -> dict:
    ...
    for threshold in thr_iter:
        eval_result = evaluate_policy(
            "CASCADE_MDP",
            lambda thr=threshold: CASCADEPolicy(csc_alert_thr=thr),
            cfg,
            datasets,
        )
        points.append(
    ...
    return {
        "thresholds": points,
        "operating_point_threshold": float(CSC_ALERT_THRESHOLD_DEFAULT),
    }
...
def run_roc_sweep_ci(cfg: Config, datasets: list[Dict], thresholds: list[float]) -> dict:
    ...
    return {"thresholds": points, "operating_point_threshold": float(CSC_ALERT_THRESHOLD_DEFAULT)}
...
def write_roc_plot(roc_metrics: dict, destination: Path) -> None:
    ...
    ax.set_ylabel("Benchmark anomaly-event recall (%)")
    ax.set_title("CASCADE operating-point sweep over CSC alert threshold")
...
def write_multi_roc_plot(
    ...
    ax.set_xlabel("False-positive rate (%)")
    ax.set_ylabel("Benchmark anomaly-event recall (%)")
    ax.set_title("CASCADE operating-point sweep (NDWI ablation)")
    ax.grid(True, linestyle="--", linewidth=0.5, alpha=0.45)
    ax.set_xlim(left=0.0)
...
def run_additional_ablations(cfg: Config) -> dict:
    ...
    max_workers = int(
        _env_any("CASCADE_ABLATION_MAX_WORKERS", "MASFE_ABLATION_MAX_WORKERS", default="8")
    )
    ...
    fig.tight_layout()
    curve_path = BENCHMARK_OUTPUT_DIR / "additional_ablation_curves.png"
...
def run_csc_sensitivity(cfg: Config, datasets: list[Dict]) -> dict:
    ...
    for weight_scale in WEIGHT_SWEEP:
        for saturation_scale in SATURATION_SWEEP:
            evi_weight = CSC_DEFAULTS.evi_weight * weight_scale
            lst_weight = CSC_DEFAULTS.lst_weight
            ndwi_weight = CSC_DEFAULTS.ndwi_weight * weight_scale
            csc_kwargs = {
                "evi_weight": evi_weight,
                "lst_weight": lst_weight,
                "ndwi_weight": ndwi_weight,
                "evi_saturation": CSC_DEFAULTS.evi_saturation * saturation_scale,
                "lst_saturation": CSC_DEFAULTS.lst_saturation * saturation_scale,
                "ndwi_saturation": CSC_DEFAULTS.ndwi_saturation * saturation_scale,
            }
            eval_result = evaluate_policy(
                "CASCADE_MDP",
                lambda: CASCADEPolicy(),
                cfg,
                datasets,
                csc_kwargs=csc_kwargs,
            )
            ...
            cases.append(
                {
                    ...
                    "evi_saturation_sigma": round(
                        CSC_DEFAULTS.evi_saturation * saturation_scale, 3
                    ),
                    "lst_saturation_sigma": round(
                        CSC_DEFAULTS.lst_saturation * saturation_scale, 3
                    ),
                    "ndwi_saturation_sigma": round(
                        CSC_DEFAULTS.ndwi_saturation * saturation_scale, 3
                    ),
                    "science_retention_pct": round(float(recall["mean"]), 3),
                    "science_retention_ci95_low": round(float(recall["ci95_low"]), 3),
                    "science_retention_ci95_high": round(float(recall["ci95_high"]), 3),
...
def report(metrics: dict, ablation_metrics: dict, roc_metrics: dict, csc_sensitivity: dict) -> None:
    ...
    print("\n" + "=" * 72)
    print("  CASCADE - An Onboard Crop Anomaly Screening, Confirmation, and Alert Downlink Engine")
    ...
    print(f"  |- Benchmark anomaly events/seed: {mc['n_dis']}")
    ...
    header = f"  {'Metric':<40} {'RAW DUMP':>10} {'FIXED OB':>10} {'CASCADE MDP':>10}"
    print(header)
    print("  " + "-" * 72)
    ...
    row("Downlink data (MB)", raw["data_mb_mean"], fixed["data_mb_mean"], cascade["data_mb_mean"])
    row("Energy consumed (Wh)", raw["energy_mean_wh"], fixed["energy_mean_wh"], cascade["energy_mean_wh"], fmt="{:.2f}")
    row("True positives (TP)", raw["tp_mean"], fixed["tp_mean"], cascade["tp_mean"], fmt="{:.1f}")
    row("False positives (FP)", raw["fp_mean"], fixed["fp_mean"], cascade["fp_mean"], fmt="{:.1f}")
    row("Priority alerts sent", 0.0, 0.0, cascade["n_alerts_mean"], fmt="{:.1f}")
    print("  " + "-" * 72)
    ...
    print("\n  CASCADE SCHEDULING DECISIONS (mean share of passes)")
    total_pct = sum(item["pct_mean"] for item in cascade["action_mix"].values())
    for action_name, action_metrics in sorted(
        cascade["action_mix"].items(),
    ...
    print("  * build/benchmark/ablation_metrics.json")
    print("  * build/benchmark/evi_only_baseline.json")
    print("  * build/benchmark/baselines_comparison_table.tex")
    print("  * build/benchmark/roc_metrics.json")
    print("  * build/benchmark/roc.png")
    print("  * build/benchmark/csc_sensitivity.json")
    print("  * build/benchmark/additional_ablations.json")
    print("  * build/benchmark/additional_ablation_curves.png")
    print("  * build/benchmark/roc_baseline_vs_no_ndwi.png")
    print(
    ...
    metrics_path = BENCHMARK_OUTPUT_DIR / "simulation_metrics.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        epilog=(
            ...
            "CASCADE_SKIP_ADDITIONAL_ABLATIONS=1. Tune: CASCADE_ADDITIONAL_ABLATION_SEEDS (default 100), "
            "CASCADE_ADDITIONAL_ABLATION_PATCHES (default 1000), CASCADE_ABLATION_MAX_WORKERS."
        ),
    )
    parser.add_argument(
        "--additional-ablations-only",
        action="store_true",
        help="Only regenerate build/benchmark/additional_ablations.json (skips headline Monte Carlo).",
    )
    parser.add_argument(
    ...
    try:
        if args.additional_ablations_only:
            start_msg = "CASCADE Simulation - running additional ablations only..."
            ...
            write_json(BENCHMARK_OUTPUT_DIR / "additional_ablations.json", additional_ablations)
            ...
            write_json(BENCHMARK_OUTPUT_DIR / "csc_sensitivity.json", csc_sensitivity)
            write_multi_roc_plot(
                ...
                BENCHMARK_OUTPUT_DIR / "roc_baseline_vs_no_ndwi.png",
            )
            ...
            start_msg = "CASCADE Simulation - running 100-seed Monte Carlo benchmark..."
            print(start_msg, flush=True)
            _benchmark_log_line(start_msg)
            ...
            csc_sensitivity = run_csc_sensitivity(cfg, datasets)
            skip_additional = args.skip_additional_ablations or _env_any(
                "CASCADE_SKIP_ADDITIONAL_ABLATIONS",
            ...
            write_json(BENCHMARK_OUTPUT_DIR / "ablation_metrics.json", ablation_metrics)
            write_json(BENCHMARK_OUTPUT_DIR / "evi_only_baseline.json", evi_only_metrics)
            write_baselines_comparison_table(
                metrics,
                ablation_metrics,
                evi_only_metrics,
                BENCHMARK_OUTPUT_DIR / "baselines_comparison_table.tex",
            )
            write_json(BENCHMARK_OUTPUT_DIR / "roc_metrics.json", roc_metrics)
            if plots_enabled:
                write_roc_plot(roc_metrics, BENCHMARK_OUTPUT_DIR / "roc.png")
            write_json(BENCHMARK_OUTPUT_DIR / "csc_sensitivity.json", csc_sensitivity)
